...
class FaceNet:
    ...
    Args:
        key: Which version of the model to use. Options are: 20180402-114759, 20180408-102900,
              20170511-185253, and 20170512-110547
        use_prebuilt: Whether to use a prebuilt Keras model. If False, a Keras model is built
                      from the TensorFlow protobuf files provided by David Sandberg.
        cache_folder: Where to save and look for model weights (defaults to ~/.keras-facenet)
    ...
    def __init__(self, key='20180402-114759', use_prebuilt=True, cache_folder='~/.keras-facenet'):
    ...
    def _normalize(self, image):
        """Normalize the image based on metadata settings."""
    ...
    def crop(self, filepath_or_image, threshold=0.95):
        ...
        Returns:
            A tuple of detections and crops.
        """
        ...
        detections = [detection for detection in self.mtcnn().detect_faces(
            image) if detection['confidence'] > threshold]
        if not detections:
            return [], []

        margin = int(0.1 * self.metadata['image_size'])
        crops = [utils.cropBox(image, detection=d, margin=margin)
                 for d in detections]
        return detections, crops

    def extract(self, filepath_or_image, threshold=0.95):
        """Extract faces and compute embeddings in one go.
        ...
        Returns:
            Same output as `mtcnn.MTCNN.detect_faces()` but enriched with an "embedding" vector.
        """
    ...
    def embeddings(self, images):
        ...
        Returns:
            Embeddings of shape (N, K) where N is the number of cropped faces and K is the dimensionality
    ...
    def lock_layers(self, layers_to_lock=None):
        """Lock specific layers of the FaceNet model to prevent gradient updates.

        Args:
            layers_to_lock: A list of layer names to lock. If None, all layers are locked.
        """
        if layers_to_lock is None:
            layers_to_lock = [layer.name for layer in self.model.layers]

        for layer in self.model.layers:
            if layer.name in layers_to_lock:
                layer.trainable = False  # Đóng không cho cập nhật gradient
            else:
                layer.trainable = True   # Cho phép cập nhật gradient
